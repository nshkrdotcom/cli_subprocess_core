...
- Update Claude model catalog (`priv/models/claude.json`) to `2026-09-17`, adding pinned `claude-fable-5-1` and `fable-5.1` shorthand for Claude Fable 5.1 with 75% discounted cache read pricing ($0.25/Mtok), and adding `mythos-5.1` (`Claude Mythos 5.1`) under restricted visibility.
