...
- if the change affects the shared session-bearing or non-local placement transport seam, put it in the execution-plane-backed transport layer
...
4. let the core route the request to the execution-plane-backed command lane or
   the execution-plane-backed transport lane as appropriate
